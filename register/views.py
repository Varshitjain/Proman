...
def companyView(request):
    comp = Company.objects.all().reverse()
    tasks = Task.objects.all()
    context = {
        'comp': comp,
        'tasks': tasks,
    }
    return render(request, 'register/companiesview.html', context)

def tasksView(request):
    tasks = Task.objects.order_by('project')
    proj = Project.objects.all()
    context = {
        'tasks': tasks,
        'proj': proj,
    }
    return render(request, 'register/tasks.html', context)
...
def profile(request):
    wp = WorkProfile.objects.all()
    if request.method == 'POST':
        img_form = ProfilePictureForm(request.POST, request.FILES)
        context = {'img_form' : img_form, 'wp' : wp, }
        if img_form.is_valid():
            img_form.save(request)
            updated = True
            context = {'img_form' : img_form, 'updated' : updated ,'wp':wp}
